...
speakers_pics = [
    ...
    "photo-1508700115892-45ecd05ae2ad", # SoundLink Revolve+
...
monitors_pics = [
    ...
    "photo-1616440347437-b1c73416efc2", # Dell P2723D 27
    "photo-1593642632559-0c6d3fc62b89", # Dell Alienware QD-OLED 34
    "photo-1563986768609-322da13575f3", # Samsung Odyssey G95SC
    "photo-1517694712202-14dd9538aa97", # Samsung Odyssey G7
    "photo-1545665277-5937489579f2"  # Samsung Smart Monitor M8
]
...
assert len(all_images) == 130
assert len(unique_images) == 130
print("PASSED LOCAL UNIQUE IMAGE CHECK: 130/130 unique image IDs!")

# Construct python code for scraper.py replacement
dict_entries = []
...
# Use the exact categories structure from scraper.py to clean and generate the mapping entries
...
for category, brands in BRANDS_CATEGORIES.items():
    ...
    for brand, models in brands.items():
        for model in models:
            # Clean duplicate brand prefix names
            clean_brand = brand
            clean_model = model
            ...
            img_id = pool[counter]
            url = f"https://images.unsplash.com/{img_id}?w=400&auto=format&fit=crop"
            dict_entries.append(f'    "{full_name}": "{url}"')
            counter += 1

dict_code = "MODEL_SPECIFIC_IMAGES = {\n" + ",\n".join(dict_entries) + "\n}"

# Read backend/services/scraper.py
with open("backend/services/scraper.py", "r", encoding="utf-8") as f:
    code = f.read()

# Replace EXACT_PRODUCT_IMAGES or MODEL_SPECIFIC_IMAGES, CATEGORY_IMAGE_POOLS, get_product_image
# Start from line 141 "EXACT_PRODUCT_IMAGES = {" or "MODEL_SPECIFIC_IMAGES = {" to line 292 "REVIEW_POOL = [" (non-inclusive)
# Let's locate the start of EXACT_PRODUCT_IMAGES and the start of REVIEW_POOL
start_marker = "MODEL_SPECIFIC_IMAGES = {"
if start_marker not in code:
    start_marker = "EXACT_PRODUCT_IMAGES = {"
    
end_marker = "REVIEW_POOL = ["
...
new_code_chunk = dict_code + "\n\n" + """def get_product_image(full_name: str, category: str) -> str:
    # Clean brand repeat in name if it exists (handles raw queries and inspection scripts)
    cleaned_name = full_name
    for brand in ["OnePlus", "Bose", "LG", "Samsung"]:
        if full_name.startswith(f"{brand} {brand} "):
            cleaned_name = full_name.replace(f"{brand} {brand} ", f"{brand} ", 1)
            break

    if cleaned_name in MODEL_SPECIFIC_IMAGES:
        return MODEL_SPECIFIC_IMAGES[cleaned_name]
    # Fallback default
    return "https://images.unsplash.com/photo-1531403009284-440f080d1e12?w=300&auto=format&fit=crop"
"""

code_updated = code[:start_idx] + new_code_chunk + "\n" + code[end_idx:]

# Also update line where full_name = f"{brand} {model}" is constructed to avoid brand repeat if not already updated
old_fullname_line = 'full_name = f"{brand} {model}"'
new_fullname_line = 'full_name = model if model.startswith(brand) else f"{brand} {model}"'

if old_fullname_line in code_updated:
    code_updated = code_updated.replace(old_fullname_line, new_fullname_line)

with open("backend/services/scraper.py", "w", encoding="utf-8") as f:
    f.write(code_updated)

print("SUCCESSFULLY MODIFIED scraper.py!")
